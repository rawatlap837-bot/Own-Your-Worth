  ...
  return (
    <section className="bg-cream py-24">
      <div className="mx-auto max-w-5xl px-6 md:px-10">
        <h2 className="animate-rise max-w-lg font-display text-3xl md:text-4xl text-plum-deep balance">
          Why Own Your Worth is different
        </h2>

        <div className="mt-14 grid gap-x-10 gap-y-12 sm:grid-cols-2">
          {differentPoints.map((item, i) => {
            const Icon = iconMap[item.icon]
            return (
              <div
                key={item.title}
                className="animate-rise border-t border-amethyst-pale pt-6"
        ...
        <div
          ...
          style={{ animationDelay: `${0.15 + differentPoints.length * 0.1 + 0.1}s` }}
        >
          <button
            type="button"
            onClick={onReserve}
